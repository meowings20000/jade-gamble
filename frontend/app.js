...
function doPolishForcePicker(st) {
  ...
  m.innerHTML = `
    ...
    ${pick(1, '輕磨', '「這料吃不消重手」——<b>磚頭料</b>，或<b>有裂紋</b>的料（每條裂把理想力度往下拉半級、深裂再拉一級）。選對了第一層成功率 <b>90%</b>（磚頭料的階梯比較短，每層漲得比好料快）。')}
    ${pick(2, '正磨', '<b>豆種、油青種</b>這種沒裂的標準料。不確定要選哪個，先選它最不容易大錯。')}
    ${pick(3, '重磨', '只有<b>冰種、玻璃種</b>這種沒裂的好料壓得住（選對時每層漲得最慢，玻璃種能一路爬到 ×8 天花板）。壓在差料或有裂的料上＝每層都在賭命。')}
    <div class="card" style="font-size:13px;line-height:1.7;margin-top:10px">
      ...
      • <b>選對力度</b>＝第一層成功率 <b>90%</b>（爆裂 10%）；<b>選錯一級每層 +18%</b>、錯兩級 +36%。<br>
      • <b>每一層都更危險</b>：從第一層 90% 一路降到最低那層 <b>25% 成功率</b>（爆裂 75%）——種水越好能爬越深，所以每層漲得越慢。<br>
      • 就算全程選對，磨到第 4 層也只剩約 <b>38%</b> 機率還站著——<b>選對只是提高成功率，不是無敵</b>。<br>
...
async function startPolish(st, force) {
  ...
  m.innerHTML = `
    ...
    <div class="kv"><span>下一層爆裂機率（越深越危險）</span><b id="pol-risk">${pct(res.break_prob)}</b></div>
