...
func PolishFeel(st *Stone, force int) string {
	...
	default:
		// 差兩級：方向要分清楚，不然「重磨壓在好料上」會誤讀成「這顆料沒用」
		if force > PolishIdealForce(st) {
			return "機器劇烈震動、石面接連崩口，聲音刺耳——不是這料不行，是你壓得太狠了，整片崩掉。"
		}
		return "機器幾乎空轉、磨面紋風不動，石屑黏成一團——力度完全不夠，這樣磨一輩子也磨不出肉。"
	}
}
